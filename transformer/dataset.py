...
def save_dataset(df, speaker_path, embed_path, save_path, dataset='train', save_option=True, frames=64):

    npz_path = save_path / 'text-audio-pose-npz'
    ...
    for i in tqdm(range(len(df))):
        try:
            ...
            if os.path.isfile(speaker_path / 'videos' / df['video_fn'][i]):
                save_voice(speaker_path, i, df, fname, frames, voice_path)
                df.at[i, 'audio_fn'] = fname + '.wav'
                audio_wav, sr = raw_repr(voice_path / (fname + '.wav'), 16000)
            else:
                continue

            # Save
            if save_option:
                np.savez(npz_path / fname, words=words, poses=poses, wvec=wvec, wav=audio_wav, face=face)
            else:
                np.savez(npz_path / fname, poses=poses, wvec=wvec, wav=audio_wav)
...
if __name__ == "__main__":
    ...
    save_path = base_path / speaker / ('train-' + str(args.frames) + '-text-audio')
    ...
    df_word = save_dataset(df_samples, speaker_path, args.wordvec_file, save_path,
                           dataset='train', save_option=False, frames=args.frames)
